...
import { entrarAMesaTorneo } from "../../helpers/fetchConnection";
import { ModalLink } from "./ModalLink";

export const MesaTorneoCard = ({ mesa }) => {
    const { uid, invitado, mesaInvitado } = useSelector((state) => state.auth);
    const { Usuarios } = require("../../usuarios.json");
    const { idMesa, jugadorUno, tipo, jugadorDos, cantidadJugadores } = mesa;
    const { connection } = useContext(SocketContext);

    const handleJugar = async (e) => {
        e.preventDefault();
        entrarAMesaTorneo(uid, idMesa, connection, jugadorUno, cantidadJugadores, jugadorDos);
    };
